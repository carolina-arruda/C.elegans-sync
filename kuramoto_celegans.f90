...
!	Program kuramoto_celegans.f90
!
!	The network contains 248 neurons and 511 gap junctions. 
!	
!	INPUT:	
!	Read the weighted adjacency matrix (netname) and communities (communities).
!	
!	Force the modforce (the user chooses what module to be forced).
!	Control parameters: lambda and force (f).
!	Choose a distribution: idist = 1 (uniform); idist = 2 (triangular); idist = 3 (Lorenztian); idist = 4 (Gaussian).
!
!	OUTPUT:
!	1. fileplots: plots order parameter r intra-module. Column 1: force; columns 2-5: r(i,i); i = module index
!	2. fileinter: plots order parameter r inter-module. Column 1: force; columns 2-6: r(i,j); i,j = module index
! 	3. intrar: plots order parameter dot-psi intra-module. Column 1: force; columns 2-5: dpsimi(i,i); i = module index; columns 6-9: error of dot psi dpsierri(i,i); i = module index
! 	4. interr: plots order parameter dot-psi inter-module. Column 1: force; columns 2-6: dpsimi(i,j); i,j = module index
!
!	Carolina A. Moreira and Marcus A. M. de Aguiar - 30/08/2018.
...
REAL*8, ALLOCATABLE, SAVE :: zx(:,:),zy(:,:)
REAL*8, ALLOCATABLE, SAVE :: rmi(:,:),dpsimi(:,:),dpsierri(:,:)
...
REAL*8 adist,xf,lambda,f,fi,ff
 ...
 CHARACTER*60 fileplots,fileinter,interr,intrar
pi = dacos(-1.0d0)
pi2 = 2.0*pi
...
!------ PARAMETERS --------
 ...
 fi = 0.0				! External forcing (initial)
 ff = 100.0			! External forcing (final)
 nf = 3				! Partition of force interval
...
! input
 ...
 communities = 'comunities_topological_10.dat'
...
END DO
 CLOSE(11)
nmod = im
write(6,*) 'Number of modules = ', nmod
...
! Choose a distribution: idist = 1 (uniform); idist = 2 (triangular); idist = 3 (Lorenztian); idist = 4 (Gaussian).
IF(idist==1) THEN
         OPEN(UNIT=10,FILE='uforced.dat',STATUS='UNKNOWN')
ELSE IF (idist==2) THEN
         OPEN(UNIT=10,FILE='tforced.dat',STATUS='UNKNOWN')
ELSE IF (idist==3) THEN
         OPEN(UNIT=10,FILE='lforced_l1e1.dat',STATUS='UNKNOWN')
ELSE
         OPEN(UNIT=53,FILE= fileplots, STATUS='UNKNOWN')
         OPEN(UNIT=54,FILE= fileinter, STATUS='UNKNOWN')
         OPEN(UNIT=57,FILE= intrar, STATUS='UNKNOWN')
         OPEN(UNIT=58,FILE= interr, STATUS='UNKNOWN')
END IF
...
sigmahat = sigma/adist
    ...
    DO k=1,nf  ! FORCE INTERVAL
    f = fi + (k-1.0)*(ff-fi)*anf
    fhat = f/adist
   ...
   loop_time: DO WHILE (x < xf) 
            ...
            IF(x > xft) THEN
                ...
                CALL ZCALC(y,it)
                ! write trajectories
                WRITE(32,130) x,(cos(y(i)),i=1,nosc)
            END IF
    END DO loop_time
    CLOSE(32)
    ntime = it
    ...
    CALL RPSIPONTOINTER
    write(6,*) 'force = ', f		
    write(53,100) f,rmi(1,1),rmi(2,2),rmi(3,3),rmi(4,4)
    write(57,100) f,dpsimi(1,1),dpsimi(2,2),dpsimi(3,3),dpsimi(4,4),dpsierri(1,1),dpsierri(2,2),dpsierri(3,3),dpsierri(4,4)
    write(54,100) f,rmi(1,2),rmi(1,3),rmi(1,4),rmi(2,3),rmi(2,4),rmi(3,4)
    write(58,100) f,dpsimi(1,2),dpsimi(1,3),dpsimi(1,4),dpsimi(2,3),dpsimi(2,4),dpsimi(3,4)
